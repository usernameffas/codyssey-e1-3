...
미션 페이지에서 제공된 공식 `data.json`을 프로젝트 루트에 포함했습니다.
...
공식 `data.json`을 실제 실행한 결과는 다음과 같습니다.
...
- `size_5_1`: Cross `0.9`, X `0.8999999999999999` → epsilon 동점 → `UNDECIDED`
- `size_13_2`: Cross `7.499999999999997`, X `7.5` → epsilon 동점 → `UNDECIDED`
- `size_25_1`: Cross `4.9`, X `4.899999999999999` → epsilon 동점 → `UNDECIDED`

위 3개는 코드나 스키마 오류가 아닙니다. 두 점수의 차이가 `1e-9`보다 작기 때문에 미션의 epsilon 정책대로 `UNDECIDED`가 되었고, expected 라벨과 다르므로 FAIL로 집계됩니다. expected에 맞추기 위해 임의로 한쪽을 선택하지 않았습니다.
...
- [x] 공식 `data.json` 추가
- [x] 공식 `data.json` 실제 실행 및 6개 결과 확인
- [ ] 실제 실행 결과 캡처
